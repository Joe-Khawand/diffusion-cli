...
def test_sdxl_default_places_on_device_without_offload() -> None:
    pipe = MagicMock()
    apply_optimizations(pipe, "mps", PipelineKind.SDXL, low_mem=False)
...
def test_low_mem_uses_offload_and_not_to() -> None:
    pipe = MagicMock()
    apply_optimizations(pipe, "mps", PipelineKind.SDXL, low_mem=True)
...
@pytest.mark.parametrize("kind", [PipelineKind.FLUX, PipelineKind.SD3])
def test_heavy_models_offload_by_default(kind) -> None:
    pipe = MagicMock()
    apply_optimizations(pipe, "mps", kind, low_mem=False)
    pipe.to.assert_not_called()
    pipe.enable_sequential_cpu_offload.assert_called_once()
...
def test_missing_methods_are_skipped() -> None:
    ...
    apply_optimizations(pipe, "cpu", PipelineKind.FLUX, low_mem=False)
    pipe.enable_sequential_cpu_offload.assert_called_once()
