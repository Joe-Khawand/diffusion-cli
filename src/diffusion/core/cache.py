...
def pull(repo_id: str) -> tuple[Path, PipelineKind]:
    """Download ``repo_id`` into the HF cache; return (snapshot_path, kind)."""
...
def list_models(*, include_all: bool = False) -> list[ModelEntry]:
    ...
    for repo in cache_info.repos:
        ...
        if not is_diffusion and not include_all:
            continue
        kind = detect_kind(snapshot_path) if is_diffusion else PipelineKind.UNKNOWN
        entries.append(
            ModelEntry(
