...
# --- Panels & tables -------------------------------------------------------
def model_ready_panel(repo_id: str, family: object, device: str, dtype: str) -> Panel:
    """Build a compact "ready" banner with the model's family, device, and dtype.
    ...
    family : object
        The detected :class:`~diffusion.core.models.ModelFamily` (its ``label`` is
        shown, falling back to ``str``).
    device : str
    ...
    """
    info = Text()
    info.append(getattr(family, "label", str(family)), style=f"bold {_GOLD}")
...
def run_with_preview(
    ...
    *,
    ...
    family : diffusion.core.models.ModelFamily
